...
fun createPuzzle(
    ...
    // (AGENTS.md, D-037).
    val pack = trayPack(tray, shapes.map { it.size }, seed)
...
fun relayout(p: Puzzle, field: Area, capPx: Double): Puzzle {
    val fresh = createPuzzle(p.sceneId, p.rows, p.cols, field, capPx, p.seed)
...
fun restorePuzzle(
    ...
    val fresh = createPuzzle(sceneId, rows, cols, field, capPx, seed)
