# Puzzlet: decision and session archive

History, not rules. `AGENTS.md` holds the rules that bind now; this file
holds why they became what they are and how each session went. Decision
numbers (D-nnn) cited in code comments resolve here. Append at the end;
never rewrite an entry except to fix a factual error, and mark a
superseded decision in place instead of deleting it.
