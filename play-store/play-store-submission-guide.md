...
app's own design language: the brand teal ground, the one die-cut piece
carrying the sailboat, and Baloo 2 lettering (the same font files the app
...
```
Puzzlet 2.7 gives the finish a better rhythm. When the last piece clicks home, the finished picture now stays on the table for a quiet second before the well done plate appears, and the bell rings with the plate instead of at the click. The last piece keeps its snap, the celebration gets its chime, and the breath belongs to the child. Everything else is the same game: five sizes, real die-cut pieces, no ads, no trackers, no permissions asked. Thank you for testing.
```
...
```
Puzzlet 2.5 adds a touch of color: the picture button on the puzzle screen now takes its tint from the picture itself, so the boat glows soft coral, the windmill soft teal, the castle soft dusk, and the button changes with every puzzle. The piece count under each picture is easier to read, and back now closes the size chooser. Everything else is the 2.4 game: sixteen pictures, five sizes, real die-cut pieces, no ads, no trackers, no permissions asked. Thank you for testing.
```
...
```
Puzzlet 2.3 puts the sound switch on the puzzle screen, right beside the picture button, so sounds can be turned off or on without leaving the game. The picture shelf is calmer without it, and the Puzzlet word at the top is a little larger. Everything else is the 2.1 game: real die-cut pieces, sixteen pictures, no ads, no trackers, no permissions asked. Thank you for testing.
```
...
1. Pull the signed AAB from the `latest-build` GitHub release into the
   ...
   folder was deleted after. For 2.2 the same check ran green: CI build
   and capture both succeeded, and the APK twin read versionCode 22,
   versionName 2.2, target 37, package io.github.muntasimulhaque.puzzlet,
   with no permission beyond the androidx core private receiver. The
   verified 2.2 AAB sat in play-store/aab/ awaiting submission, but 2.2
   was never submitted: the owner's review of its captures swapped the
   two coins and grew the shelf word, so 2.3 superseded it. For 2.3 the
   same check ran green: CI build and capture both succeeded, and the
   APK twin read versionCode 23, versionName 2.3, target 37, package
   io.github.muntasimulhaque.puzzlet, with no permission beyond the
   androidx core private receiver. The verified 2.3 AAB sat in
   play-store/aab/ awaiting submission; the owner's review of its
   captures then asked for equal finish coins, so 2.4 superseded it. For
   2.4 the same check ran green: CI build and capture both succeeded,
   and the APK twin read versionCode 24, versionName 2.4, target 37,
   package io.github.muntasimulhaque.puzzlet, with no permission beyond
   ...
   play-store/screenshots/. The verified 2.5 AAB sat in
   play-store/aab/ awaiting submission; 2.5 was submitted for review and
   the folder was deleted after. For 2.6 the same check ran green: CI
   build and capture both succeeded, and the APK twin read versionCode
