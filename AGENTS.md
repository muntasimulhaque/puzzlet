...
- The mark: the toy itself (the owner's pick, directed in D-044 and
  refined against a wooden reference). A 2x2 block of four chunky
  pieces, red gold yellow orange, big round knobs locked into matching
  sockets with dark grooves between. Generated by `:tools:makeIcons`
  across all densities plus adaptive and monochrome, verified by pixel
  probes (four bodies, the seam, mask survival), never hand-edited.
  Supersedes the S seam with socket in D-043 and the single boat piece:
  a lone piece never read as a puzzle at 48 dp.
