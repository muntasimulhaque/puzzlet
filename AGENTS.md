...
- The mark: the gather (the owner's pick from the icon-v4 candidate
  sheet, directed in D-045). Three chunky pieces, sky coral and gold,
  closing in on the honey home piece with its sockets open: the moment
  before the click. Drawn small on purpose: the whole gather sits
  inside the 66 dp mask circle, and tile art stays inside the tile
  silhouette, so no launcher shape ever truncates a piece. Generated by
  `:tools:makeIcons` across all densities plus adaptive and monochrome,
  verified by pixel probes (four bodies, mask-circle containment,
  silhouette), never hand-edited. Supersedes the wooden 2x2 block.
